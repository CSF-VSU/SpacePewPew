...
namespace SpacePewPew
{
    public class Drawer
    {
        ...
        public void Draw(LayoutManager manager, IMap map) //, ref Action act)
        {
            PointF[] coordinates;
        ...
        {
            var info = Game.Instance().GetPlayerInfo();
            ...
            Gl.glColor3f(0, 0, 0);
            Rect(5, 1, 35, 6);
            DrawString(new PointF(6,5), info.Color.ToString());
            
            //stations
            Gl.glColor3f(0, 0, 0);
            Rect(45, 1, 60, 6);
            DrawString(new PointF(46,5), info.Stations.ToString());

            //ResourceGain
            Gl.glColor3f(0, 0, 0);
            Rect(70, 1, 85, 6);
            DrawString(new PointF(71,5), "+" + info.Stations * 5);

            //ResourceCount
            Gl.glColor3f(0, 0, 0);
            Rect(95, 1, 110, 6);
            DrawString(new PointF(96,5), info.Money.ToString());

            //TimeLeft
            Gl.glColor3f(0, 0, 0);
            Rect(120, 1, 135, 6);
            DrawString(new PointF(121, 5), Additional.ConvertTime(info.TimeLeft));
        ...
        private void DrawAction(IMap map)
        {
            switch (state)
            {
        ...
        private void GetShipsFromIMap(IMap map)
        {
            for (var i = 0; i < Consts.MAP_WIDTH; i++)
                for (var j = 0; j < Consts.MAP_HEIGHT; j++)
                {
        ...
        private void Rotate(IMap map, int ShipId)
        {
        ...
        private void Move(IMap map, int ShipId)
        {
        ...
        private void Attack(IMap map, int ShipId)
        {
...
/* switch (state)
                {
                    ...
                    {
                        PointF tmp;
                        newDir = getNewDirection(ShipsInfo[ShipId].Pos, map.Path[map.Path.Count - 1]);
                            //initialization

                        if (ShipsInfo[ShipId].Direction != newDir)
                        {
                            tmp = CellToScreen(ShipsInfo[ShipId].Pos);
                            /*  if (rotateDir == -1)//(Math.Abs(ShipsInfo[ShipId].Direction - newDir) < (360 - Math.Abs(ShipsInfo[ShipId].Direction - newDir)))
                                   {
                                       elementaryRotate('-', 10, tmp.X, tmp.Y);
                                       ShipsInfo[ShipId].Direction -= 20;
                                   }
                                   else
                                   {
                                      elementaryRotate('+', 10, tmp.X, tmp.Y);
                            ...
                            if (rotateDir == -1)
                                ...
                                elementaryRotate('+', 10, tmp.X, tmp.Y);
                            ShipsInfo[ShipId].Direction += rotateDir*20;
                            if (ShipsInfo[ShipId].Direction >= 360) ShipsInfo[ShipId].Direction -= 360;
                            if (ShipsInfo[ShipId].Direction < 0) ShipsInfo[ShipId].Direction += 360;
                        }
                        else
                        {
                            //map.Refresh();
                    ...
                    {
                        Count++;
                        nextCell = map.Path[map.Path.Count - 1];
